      ...
      <CardContent className="p-6 sm:p-7 flex flex-col h-full overflow-hidden">
        ...
        <ScrollArea className="flex-1 pr-4 -mr-4 h-full">
          {gameTimeline.length === 0 ? (
            ...
            <div className="space-y-6 pb-15 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-100">
              {gameTimeline.map((item) => {
                ...
                return (
                  <div key={item.id} className="relative flex items-start gap-4 group is-active">
                    ...
                    {/* Content */}
                    <div className="flex-1 bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-sm transition-colors group-hover:bg-slate-100/50">
