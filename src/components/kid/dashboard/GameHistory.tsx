...
export default function GameHistory({ timeline }: { timeline: GameHistoryItem[] }) {
  ...
  return (
    <Card className="rounded-[32px] border-slate-200/60 dark:border-slate-800/60 bg-white dark:bg-black/30 shadow-sm h-105.75 flex flex-col overflow-hidden">
      <CardContent className="card-padding flex flex-col h-full overflow-hidden">
        <div className="flex items-center element-gap mb-6 shrink-0">
          ...
          <div>
            <h2 className="text-card-title font-black text-slate-900 tracking-tight dark:text-slate-50">
              Activity History
            </h2>
            <p className="text-body-sm leading-6 text-slate-500 font-medium dark:text-slate-400">
        ...
        <ScrollArea className="flex-1 pr-4 -mr-4 h-full">
          {gameTimeline.length === 0 ? (
            <div className="text-center text-slate-500 py-8 dark:text-slate-400">
              <Gamepad2 className="icon-lg mx-auto mb-3 opacity-20" />
              <p className="text-body-sm">No activities played yet!</p>
            </div>
          ) : (
            <div className="space-y-6 relative before:absolute before:inset-0 before:ml-5 before:-translate-x-px before:h-full before:w-0.5 before:bg-slate-100 dark:before:bg-slate-800">
                ...
                return (
                  <div key={item.id} className="relative flex items-start gap-4 group is-active">
                    ...
                    <div className="flex items-center justify-center w-10 h-10 rounded-full border-4 border-white bg-sky-50 text-sky-600 shadow-sm shrink-0 z-10 dark:border-slate-900 dark:bg-sky-950/40 dark:text-sky-400 dark:shadow-none">
                      {isTopScore ? (
                        <Trophy className="icon-sm text-amber-500" />
                      ) : (
                    ...
                    <div className="flex-1 bg-slate-50 card-padding rounded-2xl border border-slate-100 shadow-sm transition-colors group-hover:bg-slate-100/50 dark:bg-slate-950 dark:border-slate-800/80 dark:group-hover:bg-slate-800/40">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <time className="text-body-xs font-semibold text-slate-400 dark:text-slate-500">
                          {date}
                        </time>
                      </div>
                      <div className="text-body-sm font-bold leading-tight text-slate-900 dark:text-slate-50">
                      ...
                      {item.activity_settings &&
                        ...
                        item.description !== item.activity_settings.title && (
                          <div className="text-body-xs text-slate-500 dark:text-slate-400 font-medium mt-1 leading-snug">
                            {item.description}
                          </div>
                        )}
                    </div>
