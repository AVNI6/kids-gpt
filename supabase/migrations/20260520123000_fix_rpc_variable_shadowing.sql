-- 2026-05-20 12:30:00 UTC
-- Migration: Fix variable shadowing in link_users_by_email RPC
...
BEGIN
  ...
  IF v_current_role = 'kid' AND v_target_profile.role = 'parent' THEN
    ...
    VALUES (v_target_profile.user_id, p_current_user_id, true, now())
  ...
  ELSIF v_current_role = 'parent' AND v_target_profile.role = 'kid' THEN
    INSERT INTO public.parent_child_link (parent_user_id, child_user_id, is_approved, created_at)
    VALUES (p_current_user_id, v_target_profile.user_id, true, now())
    ON CONFLICT (parent_user_id, child_user_id) DO NOTHING;
