-- Migration: Fix Teacher Activity Feed Duplicates
-- Date: 2026-06-05 17:00:00 UTC
...
BEGIN
  ...
  SELECT coalesce(json_agg(row_to_json(t_ordered)), '[]'::json) INTO v_feed
  FROM (
    SELECT * FROM (
      SELECT DISTINCT ON (ae.event_type, ae.actor_user_id, ae.source_type, COALESCE(ae.source_id, ae.id))
        ae.id,
        ae.event_type,
        ae.actor_user_id,
        ae.actor_role,
      ...
      ORDER BY ae.event_type, ae.actor_user_id, ae.source_type, COALESCE(ae.source_id, ae.id), ae.created_at DESC
    ) sub
