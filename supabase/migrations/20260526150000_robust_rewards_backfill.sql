-- Migration: Robust Rewards Backfill and RPC Fixes
-- Date: 2026-05-26 UTC

BEGIN;

-- Step 1: Perform robust backfill for all activities to associate source_id and map source_type
WITH mapping AS (
...
FROM mapping m
WHERE 
  (r.source_type = 'activity' OR r.source_type IS NULL OR r.source_type = '') AND (
    ...
    (r.description ILIKE '%Flashcard%' AND m.slug = 'flashcards') OR
    (r.description ILIKE '%Color Mixer%' AND m.slug = 'color-mixer') OR
    (r.description ILIKE '%' || m.title || '%')
  );

-- Step 2: Update save_kid_activity_progress RPC to check source_id IS NOT NULL instead of source_type = 'activity' for streak calculation
...
-- Step 3: Update upsert_memory_reward RPC to search for source_type = 'memory-match' or 'activity'
...
BEGIN
  ...
  -- Look up any existing reward row for this user + this World (checking either memory-match or activity for legacy safety)
  SELECT id, rewards_amount
  INTO v_reward_id, v_current_xp
  FROM public.rewards
  WHERE user_id    = p_user_id
    AND (source_type = 'memory-match' OR source_type = 'activity')
