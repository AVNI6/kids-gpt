-- Migration: Update save_kid_activity_progress RPC to return reward_id
-- Date: 2026-06-17 12:01:00 UTC
--
-- PURPOSE:
--   The current RPC returns: { success, xp_earned, score, current_streak, longest_streak }
--   We add `reward_id` to the success response so the TypeScript server action can
--   pass it directly to saveActivityReview() without a separate query.
--
-- CHANGE:
--   1. Declare v_reward_id uuid variable
--   2. Add RETURNING id INTO v_reward_id to the INSERT INTO rewards statement
--   3. Include reward_id in the json_build_object response
...
BEGIN
  ...
  -- 7. Insert the activity reward record — capture the new row's UUID
