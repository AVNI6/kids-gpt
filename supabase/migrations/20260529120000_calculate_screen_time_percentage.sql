...
BEGIN
    ...
    SELECT COALESCE(daily_limit_minutes, 60), COALESCE(is_screen_time_limit_enabled, FALSE)
    ...
    -- 3. If limit is disabled, return 0 representing unlimited visual state
    IF NOT v_limit_enabled THEN
        RETURN 0;
    END IF;

    -- 4. Prevent division by zero and return capped exhaustion percentage
    IF v_limit_minutes <= 0 THEN
        RETURN 100;
    END IF;
